...
static int sc8551_get_adc_data(struct sc8551 *sc, int channel,  int *result)
{
	...
	pr_err("ljj ADC_REG_BASE=%x channel=%d val_h=%u val_l=%u val=%u\n",ADC_REG_BASE,channel,val_h,val_l,val);
	if(sc->is_sc8551)
	{
				       ...
				       const union power_supply_propval *val)
{
	...
	switch (prop) {
	case POWER_SUPPLY_PROP_CHARGING_ENABLED:
		sc8551_enable_charge(sc, /*val->intval*/1);
		sc8551_check_charge_enabled(sc, &sc->charge_enabled);
		sc_info("POWER_SUPPLY_PROP_CHARGING_ENABLED: %s\n",
				/*val->intval*/1 ? "enable" : "disable");
		break;
	case POWER_SUPPLY_PROP_PRESENT:
					...
					const struct i2c_device_id *id)
{
	...
	pr_err("szw:sc->mode=%d\n",sc->mode);
	pr_err("szw:match->data=%d\n",*(int*)match->data);
	...
	sc8551_check_charge_enabled(sc, &s_enable);

	pr_err("sc8551 probe successfully, Part Num:%d\n!",
				sc->part_no);
